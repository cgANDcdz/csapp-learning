...
ssize_t rio_readn(int fd, void *usrbuf, size_t n) 
{
    ...
    while (nleft > 0) {
        if ((nread = read(fd, bufp, nleft)) < 0) {
            if (errno == EINTR) /* Interrupted by sig handler return */  //若是被中断，会重新调用
 ...
 */
/* $begin rio_read */
/** 从rio缓冲区读取最多n字节数据到内存usrbuf,若rio缓冲区空，则自动系统调用read填满 **/ 
static ssize_t rio_read(rio_t *rp, char *usrbuf, size_t n)
{
    int cnt;

    while (rp->rio_cnt <= 0)    /* Refill if buf is empty */   //rio缓冲区空自会自动填满
    {  
 ...
 */
/* $begin rio_readlineb */
/****   从rio缓冲区中读取一行到内存userbuf,返回读取的字节数   ****/
ssize_t rio_readlineb(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    ...
    for (n = 1; n < maxlen; n++) 
    { 
        if ((rc = rio_read(rp, &c, 1)) == 1)   //逐字节度，判断结尾
        {
	        *bufp++ = c;
            if (c == '\n') 
            {
 ...
 */
/* $begin open_listenfd */
/****      顺序：getaddrinfo =>socket =>bind =>listen           ****/
int open_listenfd(char *port) 
{
    ...
    if ((rc = getaddrinfo(NULL, port, &hints, &listp)) != 0) {             //得到服务端套接字地址信息
    ...
    for (p = listp; p; p = p->ai_next) {
        /* Create a socket descriptor */                                  //创建文件描述符
        ...
        /* Bind the descriptor to the address */                        //将套接字地址与对应的文件描述符对应起来 
    ...
    if (listen(listenfd, LISTENQ) < 0) {                               //服务器才调用listen,进行监听
        close(listenfd);
	return -1;
    }
    return listenfd;
