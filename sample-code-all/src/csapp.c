...
/* $begin errorfuns */
/* $begin unixerror */
//errno记录系统最后一次错误的代码,error.h中定义
// => 系统调用发生错误时,会自动修改errno值
...
static ssize_t rio_read(rio_t *rp, char *usrbuf, size_t n)
{
    ...
    /*读取指定字节数到用户程序的usrbuf中,当然可能遇到不足值*/
    cnt = n;          
    if (rp->rio_cnt < n)   
	    cnt = rp->rio_cnt;
...
ssize_t rio_readlineb(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    ...
    for (n = 1; n < maxlen; n++) { 
        if ((rc = rio_read(rp, &c, 1)) == 1) {   /*每次从缓冲区读取一个字节,存放到字符c*/
	        *bufp++ = c;                         /*将读取的字符放到用户程序相应内存区域*/
	        if (c == '\n') {                     /*遇到换行符*/
                n++;
     		    break;
            }
    ...
    *bufp = 0;        /*读取一行字符后,在末尾添加NULL,表示结尾; 可见最多读取maxlen-1字节*/
    return n-1;
}
/* $end rio_readlineb */
